...
class _RecordVoiceScreenState extends State<RecordVoiceScreen> {
    ...
    try {
      ...
      if (!hasPermission && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Microphone permission not granted. You can still type a Nepali reflection below.')),
        );
        return;
      }
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Recording is unavailable on this device. Type your journal instead for the demo.')),
      );
    }
  }
  ...
  Future<void> _analyze() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Your wellbeing pulse is ready. You can share it from the Share Summary screen anytime.')),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return PrimaryShell(
      ...
      child: Column(
        ...
        children: [
          const SectionCard(
            child: Text('Speak in Nepali or type below. Sathi keeps the flow simple and supportive for a quick check-in.'),
          ),
          ...
          SectionCard(
            child: Column(
              children: [
                ...
                Text(_isRecording ? 'Recording… tap to stop' : 'Tap to record a voice journal'),
                const SizedBox(height: 12),
                ElevatedButton(
                ...
                if (_audioPath != null) ...[
                  ...
                  Text('Audio captured for demo upload ✓', style: Theme.of(context).textTheme.bodySmall),
                ],
              ],
            ),
          ),
          ...
          ElevatedButton(
            ...
            child: Text(_isProcessing ? 'Creating emotional summary…' : 'Create wellbeing pulse'),
          ),
          const SizedBox(height: 16),
          if (latest != null) JournalResultCard(entry: latest),
        ],
      ),
    );
  }
